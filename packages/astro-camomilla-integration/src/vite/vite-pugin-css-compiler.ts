  ...
  return {
    ...
    async load(id: string): Promise<string | undefined> {
      if (id === resolvedVirtualModuleId) {
        ...
        if (!isScss && !isCss) {
          throw new Error(`Astro integration styles index must be a .scss or .css file.`)
        }

        const resolvedCssCompilerId = await this.resolve(stylesIndex)

        if (!resolvedCssCompilerId) {
          throw new Error(`Styles mapping is wrong. Please add your styles to your styles folder.`)
        } else {
